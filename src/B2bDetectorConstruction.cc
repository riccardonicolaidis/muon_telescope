 ...
 fCheckOverlaps(true)
{
  ...
  fMessenger_2 = new G4GenericMessenger(this, "/Angle/","Choose the configuration of the instrument");
  AngleSet = 0*deg;
    DefineMaterials();
}
...
{
  fMessenger_2 -> DeclareProperty("Angle", AngleSet , "Select the configuration");
  // Define materials
...
G4VPhysicalVolume* B2bDetectorConstruction::DefineVolumes()
{
  ...
  G4double worldLength = 2*m;//1.2 * (2*targetLength + trackerLength);
  ...
  */
  ...
  G4double plane_position[4]={0.*cm,6.*cm,24.*cm,35.*cm};
